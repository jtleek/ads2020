...
`r cat(prettyNum(G, big.mark=",",scientific=FALSE))` of 
groups each with `r N` observations. For each group we
generate `r N` observations, which are stored in the 
...
We see a correlation of `r signif(max(res$r), 3)` and if
...
`r G` random correlations that are expected to
be 0 but have a standard error of 
`r signif(sd(res$r),3)`, the largest one will be
close 1. 
